...
namespace PIDController {
  ...
  { 3, 3, 0, 8, 7700, 8000 }};
    ...
    class ControlGains{
        ...
        uint16_t kd;
        uint16_t ki;
        void InitialControlGainsSensor(SensorStatus ss, SensorState st, PIOCState pstate){    
            if (ss == 'OK' && (st.pressure> (1.27*st.minPSI)) && pstate.mode == 'RUNNING')
            {
                kp = 0.65;
                ki = 0.56;
                kd = 0.34;}
            else if (ss == 'OK' &&(st.pressure <= 0.85 *st.maxPSI ) && pstate.mode == 'RUNNING')
            {
                kp = 0.54;
                kd = 0.67;
                ki = 0.43;}
        ...
        void InitialControlGainsValve(ValveStatus vs, ValveState vt, PIOCState pstate, SensorState st){
            if (vs == 'OK' && pstate.mode == 'STARTING'){ //Checks for error at the start.
        ...
        int16_t ControllerComp(SensorState st, ValveState vt ){
            uint16_t ms = PIOC_Timer::milliseconds;
            uint16_t sum = 0;
            uint16_t dsum = 0;
            ...
            if (vt.isOn == true){
                for (int i = 0; i++; i<= vt.onTime){
                    if (i > 0){
                        ...
                        sum = sum + kp*error[i] + kd*deriv + ki*integral; //Computing Controller
                    }
                    else {
                        uint16_t t = ms-vt.msLast;
                        pressure[i] = st.pressure;
                    ...
                    for (int j = 0; j++; j<=3){
                        if (valves[j][2] == 0){ //checking state of valve
                            if (valves[j][4] <=  ms < valves[j][5]){
                                int modulation = st.pressure;
                                if ( (vt.pin == valves[j][3]) && ((ms - valves[j][4])<= (valves[j][5] - ms) )){
                    ...
                    for (int i = 0; i++; i<= vt.offTime){
                        if (i > 0){
                            ...
                            pressure[i] = st.pressure;
                            error[i] = pressure[i] - pressure[i-1];
                            if (((error[i] - error[i-1])>0.04) && (vt.offTime- i)<=1000){
                                vt.isOn = true;     //We are checking the last one second and if there is an error.  We force the valve to be open. 
                                kp = kp* 1.25;
                                ki = ki* 1.19;
                            }
